...
// WithCustomHeaders adds h to every request the AuthClient sends under ctx.
// If already set, skip re-applying the headers.
func WithCustomHeaders(ctx context.Context, h http.Header) context.Context {
	if len(h) == 0 {
		return ctx
	}
	if _, ok := ctx.Value(customHeadersKey{}).(http.Header); ok {
		return ctx
	}
