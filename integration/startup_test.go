			...
			if err == nil {
				...
				if tc.expectedErrorStr == "" {
					...
					if err != nil && tc.expectedErrorStr != "" {
						select {
						case containsErr := <-errMatchCh:
							if !containsErr {
								t.Fatalf("snapshotter startup expected to fail with string \"%v\" but got different error", tc.expectedErrorStr)
							}
						default:
							// We should never end up here, but just in case
							t.Fatalf("snapshotter startup expected to fail but no failure reported")
						}
					}
				}
			}
