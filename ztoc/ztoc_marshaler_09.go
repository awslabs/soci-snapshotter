...
import (
	...
	ztoc_flatbuffers "github.com/awslabs/soci-snapshotter/ztoc/fbs/v09"
	flatbuffers "github.com/google/flatbuffers/go"
	"github.com/opencontainers/go-digest"
)

func flatbufToZtoc09(flatbuffer []byte) (z *Ztoc, err error) {
	defer func() {
		if r := recover(); r != nil {
			z = nil
	...
	z.Version = string(ztocFlatbuf.Version())
	z.BuildToolIdentifier = string(ztocFlatbuf.BuildToolIdentifier())
	z.CompressedArchiveSize = compression.Offset(ztocFlatbuf.CompressedArchiveSize())
	z.UncompressedArchiveSize = compression.Offset(ztocFlatbuf.UncompressedArchiveSize())
	...
	z.CompressionInfo.MaxSpanID = compression.SpanID(compressionInfo.MaxSpanId())
	z.CompressionInfo.SpanDigests = make([]digest.Digest, compressionInfo.SpanDigestsLength())
	for i := 0; i < compressionInfo.SpanDigestsLength(); i++ {
		dgst, _ := digest.Parse(string(compressionInfo.SpanDigests(i)))
		z.CompressionInfo.SpanDigests[i] = dgst
	}
	z.CompressionInfo.Checkpoints = compressionInfo.CheckpointsBytes()
	return z, nil
}

func ztocToFlatbuffer09(ztoc *Ztoc) (fb []byte, err error) {
	defer func() {
		if r := recover(); r != nil {
			fb = nil
	...
	for i := len(ztoc.TOC.Metadata) - 1; i >= 0; i-- {
		...
		metadataOffsetList[i] = prepareMetadataOffset09(builder, me)
	}
...
func prepareMetadataOffset09(builder *flatbuffers.Builder, me FileMetadata) flatbuffers.UOffsetT {
	...
	xattrs := prepareXattrsOffset09(me, builder)
...
func prepareXattrsOffset09(me FileMetadata, builder *flatbuffers.Builder) flatbuffers.UOffsetT {
