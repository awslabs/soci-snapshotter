...
// Package store implements a containers/storage "additional layer store" (ALS)
// backed by SOCI's FUSE layer core (fs/layer). It exposes a single FUSE mount
// whose tree matches the layout containers/storage (CRI-O / Podman) expects:
//
//	<mountpoint>/<base64(image-ref)>/<TOC-digest>/{diff,info,blob,use}
//
// This file is a near-1:1 port of stargz-snapshotter's store/fs.go, swapping
// stargz's fs/layer for soci's.
package store
...
// Mount mounts the ALS FUSE tree rooted at a rootnode onto mountpoint. It
// returns after the mount is ready; the server keeps serving in a goroutine.
...
type fs struct {
	...
	// inodes numbers of nodes inside each `diff` directory are prefixed by an unique uint32
...
func (n *rootnode) Lookup(ctx context.Context, name string, out *fuse.EntryOut) (*fusefs.Inode, syscall.Errno) {
	...
	if cn := n.GetChild(name); cn != nil {
		...
		case *refnode:
			copyAttr(&out.Attr, &tn.attr)
		default:
			log.G(ctx).Warn("rootnode.Lookup: unknown node type detected")
			return nil, syscall.EIO
		}
...
func (n *refnode) Lookup(ctx context.Context, name string, out *fuse.EntryOut) (*fusefs.Inode, syscall.Errno) {
	...
	if cn := n.GetChild(name); cn != nil {
		...
		case *layernode:
			copyAttr(&out.Attr, &tn.attr)
		default:
			log.G(ctx).Warn("refnode.Lookup: unknown node type detected")
			return nil, syscall.EIO
		}
		out.Ino = cn.StableAttr().Ino
		return cn, 0
...
func (n *layernode) Lookup(ctx context.Context, name string, out *fuse.EntryOut) (*fusefs.Inode, syscall.Errno) {
	if cn := n.GetChild(name); cn != nil {
		...
		case *blobnode:
			...
			found = true
		default:
			log.G(ctx).Debug("layernode.Lookup: unknown node type detected; trying NodeGetattrer provided by layer root node")
		...
		if !found {
			log.G(ctx).Warn("layernode.Lookup: unknown node type detected")
			return nil, syscall.EIO
		}
		out.Ino = cn.StableAttr().Ino
		return cn, 0
	...
	case layerLink, blobLink:
		...
		if err != nil {
			...
			log.G(ctx).WithField("digest", n.digest).WithError(err).Warnf("failed to mount layer %q: %q", name, n.digest)
			return nil, syscall.EIO
		}
		if name == blobLink {
		...
		err = func() error {
			...
			root, err := l.RootNode(id, idtools.IDMap{}) // no ID mapping
			if err != nil {
				return err
			}
		...
		if err != nil || errno != 0 {
			log.G(ctx).WithField("layerdigest", n.digest).WithError(err).WithField("errno", errno).Debugf("failed to get root node")
			if errno == 0 {
				errno = syscall.EIO
			}
			return nil, errno
...
// blobfile is the file handle of blob contents.
...
// MemRegularFileOnForget is an in-memory regular file node that releases its
// inode ID on forget.
...
// MemSymlinkOnForget is an in-memory symlink node that releases its inode ID
// on forget.
