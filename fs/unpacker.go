...
func (la *layerArchive) Apply(ctx context.Context, root string, r io.Reader, opts ...archive.ApplyOpt) (int64, error) {
	...
	if la.uncompressed != nil {
		pr, pw = io.Pipe()
		tr := AsyncTeeReader(decompressReader, pw, la.bufferPool)
		// Benchmark suggests that we should give the larger buffer to the slower consumer.
		// Digest calculation is faster than untar operation, thus it directly conumes
		// from the decompressed stream.
		la.uncompressed.AsyncVerify(io.NopCloser(tr))
		reader = pr
	}
