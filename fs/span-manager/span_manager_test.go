	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			for i := 0; i < int(ztoc.CompressionInfo.MaxSpanID); i++ {
				...
				if rdr.errCount != min(tc.spanManagerRetries, tc.readerErrors) {
					t.Fatalf("retry count is unexpected; expected %d, got %d", min(tc.spanManagerRetries, tc.readerErrors), rdr.errCount)
				}
			}
		})
	}
