...
// SOCI artifact metadata that implments the RemoteArtifactStore interface.
...
func (db *ArtifactsDb) GetIndexArtifactEntries(indexDigest string) ([]artifacts.ArtifactEntry, error) {
	artifactEntries := []artifacts.ArtifactEntry{}
	err := db.Walk(func(ae *artifacts.ArtifactEntry) error {
		if ae.Type == artifacts.ArtifactEntryTypeIndex && ae.OriginalDigest == indexDigest {
...
func (db *ArtifactsDb) Walk(f func(*artifacts.ArtifactEntry) error) error {
...
func (db *ArtifactsDb) addNewArtifacts(ctx context.Context, blobStorePath string, cs content.Store) error {
	...
	return filepath.WalkDir(blobStorePath, func(path string, d fs.DirEntry, err error) error {
		...
		if ae == nil {
			...
			indexEntry := &artifacts.ArtifactEntry{
				Size:           info.Size(),
				Digest:         indexDigest,
			...
			for _, zt := range sociIndex.Blobs {
				...
				ztocEntry := &artifacts.ArtifactEntry{
					Size:           zt.Size,
...
func (db *ArtifactsDb) GetArtifactEntry(digest string) (*artifacts.ArtifactEntry, error) {
	var entry *artifacts.ArtifactEntry
	err := db.db.View(func(tx *bolt.Tx) error {
		var err error
...
func (db *ArtifactsDb) getArtifactEntry(tx *bolt.Tx, digest string) (*artifacts.ArtifactEntry, error) {
	bucket, err := getArtifactsBucket(tx)
	if err != nil {
		return nil, err
	}
...
func (db *ArtifactsDb) GetArtifactType(digest string) (artifacts.ArtifactEntryType, error) {
...
func (db *ArtifactsDb) WriteArtifactEntry(entry *artifacts.ArtifactEntry) error {
	if entry == nil {
...
func (db *ArtifactsDb) writeArtifactEntry(tx *bolt.Tx, entry *artifacts.ArtifactEntry) error {
...
func getArtifactEntryByDigest(artifacts *bolt.Bucket, digest string) (*artifacts.ArtifactEntry, error) {
...
func loadArtifact(artifactBkt *bolt.Bucket, digest string) (*artifacts.ArtifactEntry, error) {
	ae := artifacts.ArtifactEntry{Digest: digest}
	...
	ae.Type = artifacts.ArtifactEntryType(artifactBkt.Get(bucketKeyType))
...
func putArtifactEntry(artifacts *bolt.Bucket, ae *artifacts.ArtifactEntry) error {
	if artifacts == nil {
